...
def lint_file(filepath_abs, config: LinterConfig, file_content_raw=None):
    ...
    is_template_file = "standards/templates/" in normalized_filepath_for_template_check or                        os.path.basename(normalized_filepath_for_template_check).startswith("tpl-")
    ...
    if frontmatter_str is None:
        ...
        is_tool_subfile = "tools/" in normalized_filepath_for_template_check
        is_report_file = "tools/reports/" in normalized_filepath_for_template_check
        is_standards_templates_readme = "standards/templates/README.md" == normalized_filepath_for_template_check
        is_standards_readme = "standards/README.md" == normalized_filepath_for_template_check
...
def main():
    ...
    parser.add_argument("--directory", default="standards/src",
                        help="Directory to lint (relative to repository root). Default: standards/src")
    ...
    create_dummies_for_testing = not (sys.argv[1:] and args.directory != "standards/src") # Basic check if run with default dir and no other args
