...
*   **L2 Orchestrator Plugins:** The conductors for complex symphonies. When a task requires multiple L3 Plugins to work together in a sequence, an L2 Orchestrator Plugin manages that workflow. These are not simple, predefined scripts; typically, L2 Orchestrators are sophisticated, LLM-powered agents or workflows capable of intelligently assessing the task, dynamically selecting and sequencing L3 Capability Plugins, and adaptively managing the flow based on real-time conditions. By making these intelligent orchestrators themselves Plugins, HMA allows even complex, AI-driven process logic to be modular and replaceable.
